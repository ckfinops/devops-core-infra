resource "local_file" "private_key" {
  ...
  filename = "${path.module}/c3ops_prod_keypair.pem"
}

resource "aws_s3_object" "private_key" {
  bucket = var.s3_bucket_name
  key    = "c3ops_prod_keypair.pem"
...
resource "aws_network_acl" "private" {
  vpc_id = aws_vpc.main.id
  #subnet_ids = [aws_subnet.private_weblayer[*].id, aws_subnet.private_applayer[*].id, aws_subnet.private_dblayer[*].id]
